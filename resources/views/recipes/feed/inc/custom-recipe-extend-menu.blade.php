...
@endphp
<a href="{{$urlRoute}}"
   aria-label="{{ $item->originalRecipe->title.' *'.trans('common.edited').'*' }}">
    <img src="{{ $item->originalRecipe->image->url('thumb') }}"
         alt="{{ $item->originalRecipe->title.' *'.trans('common.edited').'*' }}"
         class="week-grid_item_img">
</a>

<div class="week-grid_item_extend">
    <div class="week-grid_item_extend_content">
        <a href="{{$urlRoute}}" class="week-grid_item_extend_content_title">
        ...
        <div class="week-grid_item_extend_content_info">
            <div class="week-grid_item_extend_content_info_time">
				<span class="week-grid_item_extend_content_info_time_text">
					@if(!is_null($item?->originalRecipe?->cooking_time))
                        {{ $item->originalRecipe->cooking_time . ' ' . trans('common.' .$item->originalRecipe->unit_of_time) }}
                    @else
                        -
                    @endif
				</span>
            ...
            <div class="week-grid_item_extend_content_info_favourites">
                <favorite :recipe="{{ $item->originalRecipe->id }}"
