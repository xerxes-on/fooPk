...
DEPLOYMENT_ID=$(aws deploy create-deployment --application-name "${AWS_CODEDEPLOY_APP}" --deployment-config-name CodeDeployDefault.OneAtATime --s3-location bucket="${AWS_S3_BUCKET}",key=packages/"${BITBUCKET_COMMIT}".zip,bundleType=zip --deployment-group-name "${AWS_CODEDEPLOY_GROUP_STATIC}" --query 'deploymentId' --output text)
