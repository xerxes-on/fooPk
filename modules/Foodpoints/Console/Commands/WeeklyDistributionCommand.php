...
/**
 * Weekly foodpoints distribution
 * @deprecated
 * @package Modules\Foodpoints\Console\Commands
 */
class WeeklyDistributionCommand extends Command
{
    use DispatchesJobs;

    protected $signature   = 'foodpoints:weekly-distribution {--force : skip release date} {--silent : Distribute without notification} {--dry-run : Simulate the command without creating real jobs}';
    protected $description = 'Distribute weekly foodpoints';
    ...
    protected int $foodpointsAmount = 20;
    ...
    public function handle(): void
    {
        ...
        if (!$this->force && $now->lt(Carbon::createFromFormat('Y-m-d H:i:s', config('foodpoints.distributions.weekly.start_at')))) {
            throw new PublicException('Too early to start, starts at ' . config('foodpoints.distributions.weekly.start_at'));
        }
        ...
        $notificationTypeSlug = NotificationTypeSlugEnum::FOODPOINTS_DISTRIBUTION_WEEKLY->value;
        ...
        if (!$this->dryRun && !empty($usersId)) {
            ...
            $notificationData               = config('foodpoints.distributions.weekly.pushnotification.texts');
            ...
            $depositText = config('foodpoints.distributions.weekly.deposit_text');
                ...
                function ($users) use (&$allowedUsers, $depositText) {
                    foreach ($users as $user) {
                        try {
                            ...
                            //                                "User $user->email (#$user->id) did not receive weekly foodpoints due to error. Please assign manually.",
                            //                                'Weekly foodpoints distribution error for user'
                            //                            );
                        }
                    }
                }
    ...
    private function setUpDefaults(): void
    {
        ...
        $this->distributionType = FoodpointsDistributionTypeEnum::REGULAR_WEEKLY->value;
        $this->period = config('foodpoints.distributions.weekly.checkpoint_period');
        $this->checkpointDate = Carbon::now()->endOfDay()->subDays($this->period);
        $this->foodpointsAmount = config('foodpoints.distributions.weekly.amount');
    }
}
